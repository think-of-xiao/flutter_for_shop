...
class AppRegisterPage extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    ...
    return AppRegisterPageState();
  }
}

class AppRegisterPageState extends State<AppRegisterPage> {
  ...
  bool _timerCanClick = true, _isCheck = false;
  ...
  void initState() {
    ...
    joinController = TextEditingController();
    super.initState();
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              buildContainer(
                  '手机号', '请输入您的手机号', phoneController, TextInputType.phone),
              Container(
              ...
              buildContainer('密码', '输入6~16位密码', pws1Controller,
                  TextInputType.emailAddress),
              buildContainer(
                  '确认密码', '再次输入密码', pws2Controller, TextInputType.emailAddress),
              buildContainer(
                  '邀请码', '输入邀请人手机号', joinController, TextInputType.phone),
              CheckboxListTile(
                  value: _isCheck,
                  selected: false,
                  title: Text.rich(
  ...
  Container buildContainer(
      var title, var tipStr, var controller, var textType) {
    return Container(
      padding: EdgeInsets.only(top: 5.0, bottom: 5.0, left: 5.0),
      decoration: BoxDecoration(
          color: CupertinoColors.white,
        ...
        children: <Widget>[
          ...
          Expanded(
              ...
              child: CupertinoTextField(
                  ...
                  maxLength: 16,
                  padding: EdgeInsets.only(left: 25.0, top: 6.0, bottom: 6.0)))
        ],
      ),
    );
  }
...
//            DES_KEY, utf8.decode(base64.decode(PUBLIC_KEY_STR)));
        ...
        var desEncodeHexTypeNumber =
            await FlutterDes.encryptToBase64('11111111', DES_KEY, iv: DES_IV);
  ...
  _userRegister() {
    ...
    String joinStr = joinController.text.trim();
    ...
    } else if (joinStr.isEmpty || joinStr.length < 11) {
      MyToastUtils.showToastInCenter('请填写邀请人手机号', 0);
    } else if (!_isCheck) {
      MyToastUtils.showToastInCenter('请认真阅读注册协议并勾选', 0);
    } else {
      _showNetWaitingDialog();

      AppHttp.getInstance().post(servicePathAPI['register'], data: {
        'login_phone': accountPhoneStr,
        'client_password': generateMd5(accountPws1Str),
        'type_number': '11111111',
        'mobileCode': smsCodeStr,
        'invite_number': joinStr
      }).then((value) {
        _dismissNetWaitingDialog();
