...
class AppHttp {
  ...
  static AppHttp getInstance() {
    if (null == instance) instance = new AppHttp();
    return instance;
  }
  ...
  Future get(url, {data, options, cancelToken}) async {
    ...
    params.addAll({'brand_number': Config.brandNumber, 'phone_sys': Config.phoneOs});
    print('~~~~~~~get请求参数~~~~~~~\t\n${params.toString()}');
    Response response;
    try {
...
//      response.statusCode; 状态码
  ...
  Future post(url, {data, options, cancelToken}) async {
    ...
    Response response;
    try {
      response = await dio.post(url, queryParameters: params, options: options, cancelToken: cancelToken);
